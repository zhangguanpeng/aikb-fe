...
const NewChatPage = () => {
  ...
  const { chatData, currentChat, fetchEditChatName, fetchCreateChat } = newChatStore;
  ...
  useEffect(async () => {
    ...
    const params = {
		...
		await fetchCreateChat(params).then(() => {
      if (fromHomeValue) {
        getChatStream(fromHomeValue);
      }
    });
  ...
  const renderChatItem = (chatInfo) => {
    ...
    if (chatInfo.anwser.recommend) {
      const recommendElements = chatInfo.anwser.recommend.map((recommendItem, index) => (
        <div className="recommend-item" key={index} onClick={() => { getChatStream(recommendItem) }}>{recommendItem}</div>
      ));
  ...
  const getChatStream = (question) => {
    ...
    fetchEventSource(`aikb/v1/chat/${currentChat.id}/enhanced`, {
      method: 'POST',
      headers: {
    ...
    <div className={textReferenceDetailShow ? 'new-chat-page page-position-drawer' : 'new-chat-page page-position'}>
      <div className="head">
        <div className="chat-name">
          ...
          <span className="edit-icon" onClick={() => { setEditChatNameShow(true) }}><EditOutlined /></span>
        </div>
        <div className="reference-btn" onClick={() => { setTextReferenceDetailShow(true); }}>
      ...
      <div className="ask-input">
        ...
        />
        <div className="btns">
        <div className="btn-send" onClick={() => { getChatStream() }}>
      ...
      <Drawer
        title="引用内容"
        placement="right"
        onClose={() => { setTextReferenceDetailShow(false) }}
